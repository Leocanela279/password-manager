...
const register = async () => {
  ...
  try {
    await window.cinnamon.register({
    ...
    errorMessage.value = 'No se pudo crear la cuenta. Puede que el usuario o correo ya exista.'
  } finally {
    loading.value = false
  }
}
...
<template>
  <div class="app-shell">
    <div class="mx-auto flex min-h-screen max-w-6xl items-center justify-center px-5 py-8">
      <div class="grid w-full max-w-5xl overflow-hidden rounded-[30px] border border-white/8 bg-slate-950/40 shadow-[0_24px_90px_rgba(2,8,20,0.52)] lg:grid-cols-[0.92fr_1fr]">
        <section class="relative flex items-center justify-center px-5 py-6 md:px-8 md:py-8">
          ...
          <div class="panel-surface relative w-full max-w-lg rounded-[26px] p-6 md:p-8">
            ...
            <div class="text-center">
              <h1 class="text-4xl font-semibold tracking-tight text-slate-50">Crear Bóveda</h1>
              <p class="mt-3 text-lg text-slate-400">Abre tu espacio seguro y empieza a organizar credenciales.</p>
            </div>

            <form class="mt-8 space-y-5" @submit.prevent="register">
              <div class="space-y-2.5">
                <label for="username" class="text-base font-medium text-slate-200">Nombre de usuario</label>
              ...
              <div class="space-y-2.5">
                ...
                <div class="field-surface flex items-center gap-3 rounded-[18px] px-4 py-3.5 transition focus-within:border-cyan-300/35">
                  ...
                  <input
                    ...
                    required
                    placeholder="Define una contraseña fuerte"
                    class="w-full bg-transparent text-base text-slate-100 outline-none placeholder:text-slate-500"
                  >
                  <button
                    type="button"
            ...
            <p class="mt-6 text-center text-base text-slate-400">
              ¿Ya tienes cuenta?
              <router-link to="/login" class="font-medium text-cyan-300 transition hover:text-cyan-200">
                Iniciar sesión
              </router-link>
            </p>
          </div>
        ...
        <section class="hidden border-l border-white/8 px-10 py-10 lg:flex lg:flex-col lg:justify-between">
          <div>
            <p class="mb-4 text-sm uppercase tracking-[0.35em] text-cyan-300/70">Nueva bóveda</p>
            <h2 class="max-w-md text-4xl font-semibold leading-tight text-slate-50">
              Menos ruido visual, más control sobre cada acceso.
            </h2>
            <p class="mt-5 max-w-md text-base leading-7 text-slate-400">
              Crea tu cuenta y empieza a guardar contraseñas, enlaces y registros con una interfaz limpia y profesional.
            </p>
          </div>

          <div class="grid gap-4">
            <div class="panel-surface rounded-[22px] p-5">
              <p class="text-sm uppercase tracking-[0.24em] text-slate-500">Preparado para tu flujo</p>
              <div class="mt-4 grid gap-4">
                <div class="soft-surface rounded-[16px] p-4">
                  <p class="text-sm text-slate-400">1. Crea tu usuario</p>
                  <p class="mt-1 text-base font-medium text-slate-100">Identidad clara para acceder rápido</p>
                </div>
                <div class="soft-surface rounded-[16px] p-4">
                  <p class="text-sm text-slate-400">2. Protege tu bóveda</p>
                  <p class="mt-1 text-base font-medium text-slate-100">Contraseña maestra y organización local</p>
                </div>
                <div class="soft-surface rounded-[16px] p-4">
                  <p class="text-sm text-slate-400">3. Administra credenciales</p>
                  <p class="mt-1 text-base font-medium text-slate-100">Panel oscuro, ordenado y más serio</p>
                </div>
              </div>
            </div>
          </div>
